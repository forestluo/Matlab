function [pointMarks,treeNodes] = MeanMainsplitter(xValues,yValues,depth)
    %预处理检测
    ...
    %设置树节点数值
    newNodes(1,:) = [xMean,yMean,beta,theta];
    %计算判别式值
    k = tan(beta);
    ...
    while depth > 1
        depth = depth - 1;
        [newMarks,newNodes] = MeanSubsplitter(xValues,yValues,newMarks,newNodes);
    end
    %设置数值，继续迭代
