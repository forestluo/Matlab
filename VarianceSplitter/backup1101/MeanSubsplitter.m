function [newMarks,newNodes] = MeanSubsplitter(xValues,yValues,pointMarks,treeNodes)
    %树深度
    %至少要有一个节点
    depth = 1;
    %预处理检测
    ...
    for i = 1:2^depth
        ...
        %获得分割结果
        [xNewMean,yNewMean,beta,theta] = MeanSplit(xNewValues,yNewValues);
        %设置节点数值
        row = row + 1;
        newNodes(row,:) = [xNewMean,yNewMean,beta,theta];
        %计算判别式值
        k = tan(beta);
